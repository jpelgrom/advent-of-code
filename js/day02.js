...
function partOne() {
    ...
    for (let i = 0; i < rules.length; i++) {
        const minimumOccurences = rules[i].match(/[0-9]*(?=-)/)[0];
        const maximumOccurences = rules[i].match(/(?<=-)[0-9]*(?=\s)/)[0];
        const letterRequired = rules[i].match(/(?<=\s)[a-z]*(?=:)/)[0];
        const password = rules[i].match(/(?<=:\s)[a-z]*/)[0];
        const occurences = [...password.matchAll(new RegExp(letterRequired, "g"))].length;

        if (occurences >= minimumOccurences && occurences <= maximumOccurences) numberOfValidPasswords++;
    }
...
function partTwo() {
    ...
    let numberOfValidPasswords = 0;
    for (let i = 0; i < rules.length; i++) {
        const positionOne = rules[i].match(/[0-9]*(?=-)/)[0];
        const positionTwo = rules[i].match(/(?<=-)[0-9]*(?=\s)/)[0];
        const letterRequired = rules[i].match(/(?<=\s)[a-z]*(?=:)/)[0];
        const password = rules[i].match(/(?<=:\s)[a-z]*/)[0];

        const validOnOne = password.substr(positionOne - 1, 1) == letterRequired;
        const validOnTwo = password.substr(positionTwo - 1, 1) == letterRequired;
        if ((validOnOne || validOnTwo) && !(validOnOne && validOnTwo)) numberOfValidPasswords++;
    }
    return numberOfValidPasswords;
}
