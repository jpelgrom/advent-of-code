...
function partOne() {
    const scannedDocs = getScannedDocs();
    let validDocs = 0;
    scannedDocs.forEach((item) => {
        if ("byr" in item && "iyr" in item && "eyr" in item && "hgt" in item
            && "hcl" in item && "ecl" in item && "pid" in item) { // all except cid are required
            validDocs++;
        }
    });
    return validDocs;
...
function partTwo() {
    ...
    let validDocs = 0;
    scannedDocs.forEach((item) => {
        if ("byr" in item && item.byr.length == 4 && item.byr >= 1920 && item.byr <= 2002
            && "iyr" in item && item.iyr.length == 4 && item.iyr >= 2010 && item.iyr <= 2020
            && "eyr" in item && item.eyr.length == 4 && item.eyr >= 2020 && item.eyr <= 2030
            && "hgt" in item
            && "hcl" in item && item.hcl.match(/(?<=#)[a-f0-9]{6}/) != null
            && "ecl" in item && validEcl.indexOf(item.ecl) != -1
            && "pid" in item && item.pid.length == 9) { // all except cid are required
            const foundHeightUnit = item.hgt.match(/cm|in/);
            const foundHeightValue = item.hgt.match(/[0-9]+(?=cm|in)/);
            if (foundHeightUnit != null && (foundHeightUnit[0] == "cm" ? (foundHeightValue[0] >= 150 && foundHeightValue[0] <= 193)
                : (foundHeightValue[0] >= 59 && foundHeightValue[0] <= 76))) {
                validDocs++;
            }
        }
    });
...
function getScannedDocs() {
    ...
    for (let i = 0; i < inputLines.length; i++) {
        if (inputLines[i]) {
            let scannedDoc = {};
            const scannedKVs = [...inputLines[i].matchAll(new RegExp(/(?<=\s|^)[a-zA-Z0-9:#]*(?=\s|$)/, "g"))];
            scannedKVs.forEach((item) => {
                const scannedKV = item[0].split(":");
                scannedDoc[scannedKV[0]] = scannedKV[1];
            });

            if (typeof scannedDocs[scannedIndex] === "undefined") {
                scannedDocs[scannedIndex] = scannedDoc;
            } else {
                scannedDocs[scannedIndex] = { ...scannedDocs[scannedIndex], ...scannedDoc };
            }
        } else { // empty line, so it'll be a new document
            scannedIndex++;
        }
    }
    return scannedDocs;
